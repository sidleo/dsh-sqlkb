...
async function buildSection(dataDir, maxSectionChars) {
  ...
  text = [
    '## SQL 知识注册表（描述层）',
    `> 数据源：${dataDir}。共 ${data.tables.length} 张表、${data.examples.length} 个示例。知识库含每张表的字段清单/字段注释与已沉淀的查询示例。`,
    '> 【硬要求】做任何 SQL 相关工作第一步必须先调用 sqlkb_list 获取全量清单，看清有哪些表/示例，再决定下一步——不要直接凭印象写 SQL 或直接搜索。',
